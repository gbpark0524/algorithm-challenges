...
public class Autocomplete {
	public int solution(String[] words) {
		...
		while (!pq.isEmpty()) {
			after = pq.poll();
			l = findCommonLen(before.word, after.word);
			answer += Math.min(Math.max(before.prefix.length(), l) + 1, before.word.length());
			after.prefix = after.word.substring(0, l);
			before = after;
		}
	...
	static int findCommonLen(String str1, String str2) {
		int i;
	...
	public static void main(String[] args) {
		...
		for (int i = 0; i < answers.size(); i++) {
			TestUtil.startTimer();
			TestUtil.test(answers.get(i), new Autocomplete().solution(wordsList.get(i)));
		}
	}
}
/*
