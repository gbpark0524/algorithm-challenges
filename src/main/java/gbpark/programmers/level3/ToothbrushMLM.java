...
public class ToothbrushMLM {
	public int[] solution(String[] enroll, String[] referral, String[] seller, int[] amount) {
		int[] answer = new int[enroll.length];
		Map<String, Employee> dic = new HashMap<>();
		Employee center = new Employee();

		for (String name : enroll) {
			dic.put(name, new Employee());
		}

		for (int i = 0; i < enroll.length; i++) {
			dic.get(enroll[i]).parent = "-".equals(referral[i]) ? center : dic.get(referral[i]);
		}

		for (int i = 0; i < seller.length; i++) {
			Employee employee = dic.get(seller[i]);
			int profit = amount[i]*100;
			while (employee != center) {
				employee.money += profit - profit/10;
		...
		for (int i = 0; i < enroll.length; i++) {
			answer[i] = dic.get(enroll[i]).money;
		}
	...
	public static void main(String[] args) {
		ToothbrushMLM thisClass = new ToothbrushMLM();
		CodingTest test = new CodingTest<>(thisClass::solution, STRING_ARRAY, STRING_ARRAY, STRING_ARRAY, INT_ARRAY,  INT_ARRAY);
		test.codingTest("[\"john\", \"mary\", \"edward\", \"sam\", \"emily\", \"jaimie\", \"tod\", \"young\"]\t[\"-\", \"-\", \"mary\", \"edward\", \"mary\", \"mary\", \"jaimie\", \"edward\"]\t[\"young\", \"john\", \"tod\", \"emily\", \"mary\"]\t[12, 4, 2, 5, 10]\t[360, 958, 108, 0, 450, 18, 180, 1080]");
		test.codingTest("[\"john\", \"mary\", \"edward\", \"sam\", \"emily\", \"jaimie\", \"tod\", \"young\"]\t[\"-\", \"-\", \"mary\", \"edward\", \"mary\", \"mary\", \"jaimie\", \"edward\"]\t[\"sam\", \"emily\", \"jaimie\", \"edward\"]\t[2, 3, 5, 4]\t[0, 110, 378, 180, 270, 450, 0, 0]");
	}
}
